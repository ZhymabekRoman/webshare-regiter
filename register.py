...
def _update_random_proxy(proxy_file_name: str = "1", proxy_type: str = "socks5"):
    ...
    if not os.path.isfile(proxy_file_name):
        logger.warning("No proxy file list was found. Using Webshare without proxy can lead to multiple throttle ban.")
        return
...
def register_acc(_recaptcha_token_provider: Callable = _recaptcha_token_manual, _random_email_provider: Callable = _random_email, _random_password_provider: Callable = _random_password) -> Tuple[str, str, str]:
    random_email = _random_email_provider()
    random_password = _random_password_provider()
    while True:
      try:
        recaptcha_code = _recaptcha_token_provider()
        response = request_session.post("https://proxy.webshare.io/api/v2/register/", json={"email": random_email, "password": random_password, "recaptcha": recaptcha_code, "tos_accepted": True, "marketing_email_accepted": False})
        ...
        if response.status_code != 200:
            if response_json.get("recaptcha"):
              if response_json["recaptcha"][0]["code"] == "captcha_invalid":
                ...
                recaptcha_code = _recaptcha_token_provider.cache = None
...
def get_proxy_download_token(account_token: str) -> str:
    ...
    response = request_session.get("https://proxy.webshare.io/api/v2/proxy/config/", headers=auth_header)
    response_json = response.json()
...
def get_proxy(proxy_download_token: str) -> str:
    response = requests.get(f"https://proxy.webshare.io/api/v2/proxy/list/download/{proxy_download_token}/-/any/username/direct/-/")
...
def main():
    ...
    try:
      for i in range(1, account_count + 1):
        ...
        output = {"token": account_token, "email": email, "password": password, "proxy_list": proxy_list}
